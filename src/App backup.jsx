        ...
        <section className="dashboard-section">
          <div className="section-head">
            <div>
              ...
              <p className="section-sub">
                Tap a patient to open the complete doctor record.
              </p>
            </div>
          </div>
      ...
      <main className="page-container">
        <section className="patient-banner">
          ...
          <div className="banner-info-row">
            ...
            <div>
              <span>Urgency</span>
              <Badge level={currentPatient.urgency} />
            </div>
          </div>
        </section>
...
function Consultation({ navigate }) {
  return (
        ...
        <section className="card">
          <div className="section-head">
            <div>
              ...
              <p className="section-sub">
                Arovia consultation room · patient video call
              </p>
            </div>
          ...
          <button className="dark-btn full" onClick={() => navigate("patient")}>
...
function ShareScan({ navigate, openPatient }) {
  ...
  function findPatient(id = patientId) {
    ...
    if (match) {
      ...
      setScanStatus("Patient not found. Try ARV-001, ARV-018 or ARV-029.");
    }
  }
  ...
  async function startScanner() {
    try {
      ...
      if (!Html5Qrcode) {
        throw new Error("QR scanner is unavailable.");
      }
      ...
      await scanner.start(
        ...
        (decodedText) => {
          ...
          if (match) {
            setScanStatus(`QR recognized: ${match.id}`);
            stopScanner();
            openPatient(match);
          } else {
            setScanStatus(`QR recognized (${value}), but no matching patient was found.`);
          }
        },
        () => {}
      );
      ...
      setScanStatus(
        error?.name === "NotAllowedError"
          ...
          : "Unable to start the camera. Make sure the page is running on localhost and allow camera access."
      );
    }
  }
        ...
        <div className="hero-grid">
          <section className="card scan-card">
            ...
            <p className="login-sub center">
              Use the camera to scan a patient's Arovia QR code and open their record instantly.
            </p>

            <div className={`scanner-box real-scanner ${active ? "scanner-running" : ""}`}>
              {active ? (
                <div id="arovia-qr-reader" className="qr-reader"></div>
              ) : (
                <div className="scanner-idle">
                  <Icon name="qr" size={88} />
                  <strong>Camera QR Scanner</strong>
                  <span>Press Open Scanner to activate your camera.</span>
                </div>
              )}
            </div>
            ...
            <div className="scan-help">
              <strong>Demo QR IDs</strong>
              <span>ARV-001 · ARV-018 · ARV-029</span>
            </div>
          </section>
  ...
  return (
    <>
      ...
      {screen === "emergency" ? (
        ...
        <Consultation navigate={navigate} />
      ) : screen === "share" ? (
